...
Deployment evidence batches are published evidence. A generator MUST keep private key material and raw credentials out of every retained byte and every report: field names that can only denote such material (mnemonic, private/secret/signing keys, seed phrases, keypairs, PKCS#8/JWK encodings, bearer/access/refresh tokens, passwords) MUST cause fail-closed rejection of the batch, never silent redaction. Field names that collide with public data - `seed`/`seeds` as program-derived-address derivation labels, schema vocabulary, rendering terms - MUST NOT be name-rejected. Report detail levels (`full`/`summary`/`hash_only`) are report-sizing and narrative-authority controls, not secrecy controls: retained artifacts ship hash-bound and byte-complete in the batch at every detail level.
